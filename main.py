...
def callback_query(call):
    ...
    # 1. Обработка подтверждения имени
    if call.data == "confirm_name":
        # Имя уже есть в user_data (из user_data.get или first_name)
        if user_id not in user_data:
            user_data[user_id] = {}
        if 'name' not in user_data[user_id]:
            user_data[user_id]['name'] = call.from_user.first_name
        bot.send_message(call.message.chat.id, f'Отлично, {user_data[user_id]["name"]}!')
        ask_for_reminder_time(call.message)

    # 2. Обработка ввода другого имени
    elif call.data == "enter_other_name":
        bot.send_message(call.message.chat.id, "Напиши своё имя:")
        bot.register_next_step_handler(call.message, get_new_name)

    # 3. Обработка выбора времени напоминания
    elif call.data.startswith("set_time_"):
        time_str = call.data.split("_")[-1]  # "20:00"
        if user_id not in user_data:
            user_data[user_id] = {}
        user_data[user_id]['reminder_time'] = time_str
        bot.send_message(call.message.chat.id, f'Напоминание установлено на {time_str}.')
        send_initial_commands(call.message)

    # 4. Обработка выбора настроения (уже есть, но дополним)
        ...
        # Логирование (если user_logger есть, иначе закомментируй)
        try:
            log_user_data(user_id, user_name, mood)
        except NameError:
            pass  # если модуля нет, просто пропускаем

        if mood in ["green", "yellow"]:
            mood_messages = positive_messages   # уже загружены
        else:
            mood_messages = negative_messages

        message = random.choice(mood_messages).strip()
